...
assert_contains '"streaming": {"mode": "progress"}'
assert_contains '"ackReaction": "eyes"'
assert_contains '"typingReaction": "thinking_face"'
assert_contains 'default_msteams_channel = {'
assert_contains 'slack_channel = default_slack_channel.copy()'
assert_contains 'slack_channel.update(preserved_slack_channel)'
assert_contains '__SLACK_CHANNEL_JSON__": json.dumps(slack_channel)'
...
assert_contains "workspace_archive_b64=\$(tar -C openclaw/workspace -czf - . | base64 | tr -d '\\n')"
assert_contains 'workspace-archive:'
assert_contains 'workspace.tar.gz.b64: |'
assert_contains '$workspace_archive_b64'
...
assert_contains 'openclaw/workspace'
assert_contains 'WORKSPACE_ARCHIVE_SOURCE="/workspace-source/workspace.tar.gz.b64"'
...
assert_contains 'WORKSPACE_DOCS_SOURCE_HASH="\$(sha256sum "\$WORKSPACE_ARCHIVE_SOURCE" | awk '\''{print \$1}'\'')"'
assert_contains 'if [ "\$WORKSPACE_DOCS_SOURCE_HASH" != "\$DEPLOYED_WORKSPACE_DOCS_HASH" ]; then'
assert_contains 'base64 -d "\$WORKSPACE_ARCHIVE_SOURCE" | tar -xzf - -C /home/node/.openclaw/workspace'
