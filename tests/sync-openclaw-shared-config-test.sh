...
assert_contains 'kubectl exec -n "$NAMESPACE" "$pod_name" -c main -- mkdir -p "$CONFIG_ROOT/plugins" "$CONFIG_ROOT/skills"'
assert_contains 'kubectl cp plugins/. "$NAMESPACE/$pod_name:$CONFIG_ROOT/plugins" -c main'
assert_contains 'kubectl cp skills/. "$NAMESPACE/$pod_name:$CONFIG_ROOT/skills" -c main'
...
assert_contains "Shared plugins and skills synced to all OpenClaw deployments."
