...
assert_contains 'kubectl exec -n "$NAMESPACE" "$pod_name" -c main -- mkdir -p "$CONFIG_ROOT/plugins" "$CONFIG_ROOT/workspace/skills" "$CONFIG_ROOT/workspace"'
assert_contains 'kubectl cp openclaw/plugins/. "$NAMESPACE/$pod_name:$CONFIG_ROOT/plugins" -c main'
assert_contains 'kubectl cp openclaw/skills/. "$NAMESPACE/$pod_name:$CONFIG_ROOT/workspace/skills" -c main'
